...
export default function ShareEventModal({ isOpen, onClose, shareUrl, title }: ShareModalProps) {
    ...
    return (
        <AnimatedDialog open={isOpen} showCloseButton={false} className="md:max-w-md py-4">
            <button
                onClick={onClose}
                className="absolute right-4 size-7 flex justify-center items-center top-12 z-50 rounded-full p-1 bg-neutral-6 hover:bg-neutral-5 text-white transition-colors"
            >
                <Icon icon="iconamoon:close-duotone" width="24" height="24" className="size-7" />
            </button>

            <DialogHeader className="text-center mb-7">
                <DialogTitle className="text-xl text-center font-bold text-secondary-9">
                    Share with Friends
                </DialogTitle>
                <DialogDescription className="text-sm text-center text-secondary-7">
            ...
            <div className="space-y-8">
                ...
                <div className="space-y-4">
                    <label className="text-sm font-bold text-secondary-9 block">Share to</label>
                    <div className="grid grid-cols-5 gap-2">
                        {SOCIAL_PLATFORMS.map((platform) => (
                            <div key={platform.name} className="flex flex-col items-center gap-2">
                                ...
                                <span className="text-[10px] text-secondary-6 text-center leading-tight">
                                    {platform.name}
                                </span>
                            </div>
                        ))}
                ...
                <div className="space-y-2">
                    <label className="text-sm font-bold text-secondary-9 block">Copy link</label>
                    <div className="flex items-center bg-[#F4F5F6] border border-[#D0D5DD] rounded-xl px-4 py-3 gap-3">
                        <input
                            type="text"
                            readOnly
                            value={shareUrl}
                            className="bg-transparent border-none outline-none text-sm text-secondary-7 w-full focus:ring-0 truncate cursor-default"
                        />
                        ...
                        <button
                            ...
                            className={cn(
                                ...
                                copied
                                    ...
                                    : "text-secondary-6 hover:text-primary hover:bg-primary-1"
                            )}
                        >
